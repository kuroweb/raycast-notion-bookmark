...
import { matchKey } from "../../tags/tags";
    ...
    >
      ...
      {!error && selected.length === 0 && !isLoadingSelection ? (
        <List.EmptyView
          ...
          description="Open Configure Bookmark Databases and choose which databases to search."
          actions={
            <ActionPanel>
              <SettingsActions />
            </ActionPanel>
...
function SettingsActions() {
  return (
    <ActionPanel.Section>
      <Action
        title="Configure Bookmark Databases"
        icon={Icon.Gear}
        onAction={() => {
          launchCommand({
            name: "configure-bookmark-databases",
            type: LaunchType.UserInitiated,
          }).catch((error) => showFailureToast(error, { title: "Could not open Configure Bookmark Databases" }));
        }}
      />
